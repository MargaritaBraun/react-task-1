...
import { useActions, checkIsId, RootState } from '../../../Redux/Redux-main';
...
const Card = ({ id, title, author_name: autor, ...other }: CardProps) => {
  const { add, deleteById } = useActions();
  const countBookSelect = useSelector(
    (state: RootState) => state.bookCollections.length
  );
